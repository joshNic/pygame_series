...
class Mob(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        top = self.rect.top
        if top > HEIGHT + 10 or left < -100 or right > WIDTH + 100:
            self.rect.x = random.randrange(WIDTH - self.rect.width)
...
def show_game_over_screen():
    ...
    draw_text(screen, "Press a key to begin Game", 18, WIDTH / 2, HEIGHT * 3 / 4)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(FPS)
...
shield_sound = pygame.mixer.Sound(path.join(sound_dir, 'Pow3.wav'))
...
meteor_list = [
    ...
    'meteorBrown_big4.png', "meteorBrown_med1.png", 'meteor_mid.png',
...
for i in range(9):
    ...
    filenam = 'sonicExplosion0{}.png'.format(i)
    img = pygame.image.load(path.join(img_dir, filenam)).convert()
    img.set_colorkey(BLACK)
    # img_lg = pygame.transform.scale(img, (75, 75))
